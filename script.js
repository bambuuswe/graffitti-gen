// VÄNTA tills DOM är laddad innan vi kör kod
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (let i = 1; i <= totalImages; i++) {
    ...
    img.onload = () => {
      imagesLoaded++;
      console.log(`Doodle ${i} laddad`);
      if (imagesLoaded === totalImages) {
        console.log("Alla doodles klara!");
      }
    };
    img.onerror = () => {
  ...
  function addDoodle() {
    ...
    // Starta i mitten av skärmen med lagom storlek
    const scale = 0.3;
    ...
    showNotification(`Doodle ${slider.value} tillagd!`);
    draw();
  }
  ...
  // Doodle-knappar
  addDoodleBtn.addEventListener("click", function(e) {
    e.preventDefault();
    addDoodle();
  });
  
  removeDoodleBtn.addEventListener("click", function(e) {
    e.preventDefault();
    removeLastDoodle();
  });
  
  clearDoodlesBtn.addEventListener("click", function(e) {
    e.preventDefault();
    clearAllDoodles();
  });

  // Slider uppdaterar bara label
  slider.addEventListener("input", function() {
    label.innerText = `Doodle ${slider.value}`;
  });

  // Text-knappar
  updateTextBtn.addEventListener("click", function(e) {
    e.preventDefault();
    draw();
    showNotification("Text uppdaterad!");
  });
  ...
  resetTextBtn.addEventListener("click", function(e) {
    ...
    textInput.value = "";
    draw();
    showNotification("Text rensad");
  });

  // Navigation
  goToEditorBtn.addEventListener("click", function(e) {
    e.preventDefault();
  ...
  canvas.addEventListener("mousedown", function(e) {
    ...
    // Kolla bakifrån (sista doodlen överst)
    for (let i = doodles.length - 1; i >= 0; i--) {
      ...
      if (pos.x >= d.x && pos.x <= d.x + w &&
        ...
        offsetX = pos.x - d.x;
        offsetY = pos.y - d.y;
        // Flytta till slutet av arrayen (överst)
        doodles.splice(i, 1);
        doodles.push(d);
        draw();
        break;
  ...
  exportBtn.addEventListener("click", function(e) {
    ...
    try {
      ...
      showNotification("Bilden sparad!");
    } catch (err) {
